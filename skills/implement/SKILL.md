---
name: implement
description: Use when turning an approved blueprint design or plan into code, including dependency graphing, implementation waves, review passes, and final design-conformance verification. Use only after the user approves a design or plan.
...
workflow strictly. Use it only after the user approves a design or plan.
...
1. **Resolve the input**:
   ...
   - If the user provided a slug, look it up under
     `$BLUEPRINTS_DIR/$PROJECT/spec/` and `plan/`:
     `ls "$BLUEPRINTS_DIR/$PROJECT/"{spec,plan}"/"*"$SLUG"*`
     If multiple matches, show them and ask the user to pick one.
2. Read the design document at the resolved path.
3. Identify the **recommended approach** — only implement that approach, not alternatives.
4. Extract the key components, modules, or changes that need to be built.
...
merge, or remove tasks. For small work (one or two Small tasks), skip the
graph ceremony and proceed directly after stating what you will change.

---

## Phase 3: Execute Tasks

For each wave, execute all tasks in the wave. When subagents are available,
run independent tasks in parallel; otherwise work through the wave
sequentially yourself, keeping edits scoped and following repo patterns.

1. **With subagents — spawn one `general-purpose` agent per task** in the wave. Each agent gets:
...
3. After each substantial wave completes:
   - Summarize what each task produced (files changed, key decisions)
   - **Run three review passes** (in parallel when reviewer agents are
     available, otherwise perform them yourself), each receiving the list of
...
4. **Fix round (one round only)**: Collect all Critical and Warning issues
   ...
   - Briefly summarize what was fixed. Do **not** run another review cycle —
     one round of fixes is the limit unless the user asks for a deeper cycle.
5. Use the runtime's task-tracking tools (e.g. `TaskCreate` / `TaskUpdate`)
   when available to track progress through waves.
...
2. Run tests if a test suite exists; run typecheck and lint where configured.
3. For UI changes, verify in a browser where feasible.
...
1. Run a conformance review (use an `architect-review` agent when available,
   otherwise perform it yourself) with:
...
- **Prefer editing existing files** over creating new ones or new abstractions.
- If a task turns out to be more complex than estimated, or implementation
  uncovers a major design issue, pause and re-scope with the user rather than
  producing half-finished work.
- When subagents support isolated workspaces, run each one in a worktree
  (e.g. `isolation: "worktree"`) when tasks in the same wave edit overlapping
  files. For non-overlapping changes, run without isolation for speed.
